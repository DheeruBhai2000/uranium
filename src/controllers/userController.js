...
const createUser = async function (abcd, xyz) {
  ...
  try {
    ...
    xyz.status(500).send(error.message)
  }
};
// =================login===============
const loginUser = async function (req, res) {
  try {
    ...
    if (user) {
      ...
      let token = jwt.sign({ userId: user._id.toString(), batch: "uranium", orga: "functionup" }, "secret-this");
...
const postMessage = async function (req, res) {
  ...
  // let messag=req.params

  let updatedPosts = user.posts
  //add the message to user's posts
  updatedPosts.push(req.body.posts)
